...
function AnimEOM4 ( t , z , rj , q , u , nq , ncn )
...
parfor i = 1 : ss
    rjt(:,:,i) = subs (  rj , qu , [ z( i , 1 : nq ) ; z( i , ns + 1 : ns + nq ) ] );
end
rjt(:,:,end) = subs (  rj , qu , [ z( end , 1 : nq ) ; z( end , ns + 1 : ns + nq ) ] );
rjt = double ( rjt );
...
for i = 1 : n - 1
    ...
    if xs(1) - xs(2) ~= 0 || ys(1) - ys(2) ~= 0 || zs(1) - zs(2) ~= 0
        ...
        ln(i) = line ( xs , ys , zs , ...
           'linewidth' , 2 , 'erase' , 'xor' ,'color' , clr{rem(i,7)+1} );
